...
const Cart = () => {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:5000/api/v1/getItems?username=${username}`)
      .then((response) => {
  ...
  const handleRemoveItem = (itemId) => {
    ...
    axios.post('http://localhost:5000/api/v1/removeItem', {
      itemId,
    });
  };
  ...
  const handleCheckout = () => {
    ...
    axios.post('http://localhost:5000/api/v1/saveOrder', orderDetails)
      .then(response => {
