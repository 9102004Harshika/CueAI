...
const ForgotPassword = () => {
  ...
  const handleSendOtp = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('http://localhost:5000/api/v1/sendOtp', { email });
  ...
  const handleVerifyOtp = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('http://localhost:5000/api/v1/verifyOtp', { email, otp });
      setMessage(response.data.message);
